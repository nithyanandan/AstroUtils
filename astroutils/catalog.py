...
class SkyModel(object):
    ...
    def generate_spectrum(self, ind=None, frequency=None, interp_method='pchip'):
        ...
        if self.spec_type == 'func':
            ...
            if len(uniq_names) > 1:
                counts, edges, bnum, ri = OPS.binned_statistic(invind, statistic='count', bins=list(range(len(uniq_names))))
            else:
                counts = len(invind)
                ri = list(range(counts))
            ...
            for i, name in enumerate(uniq_names):
                ...
                if name == 'monotone':  # Needs serious testing
                    ...
                    ind2d = list(zip(ind_match, refind))
                    spectrum[list(zip(*ind2d))] = self.spec_parms['flux-scale'][ind_match]
                if name == 'power-law':
