...
been incorporated into the AstroIDL User's Library.  Function included are:
  ...
  * robust_mean - robust estimator of the mean of a data set
  * robust_mode - robust estimate of the mode of a data set using the half-sample
    method
  * robust_std - robust estimator of the standard deviation of a data set
  * robust_checkfit - return the standard deviation and biweights for a fit in order 
    to determine its quality
  * robust_linefit - outlier resistant fit of a line to data
  * robust_polyfit - outlier resistant fit of a polynomial to data
...
def robust_mean(inputData, Cut=3.0, axis=None, dtype=None):
    ...
    if axis is not None:
        fnc = lambda x: robust_mean(x, dtype=dtype)
...
def robust_mode(inputData, axis=None, dtype=None):
    ...
    if axis is not None:
        fnc = lambda x: robust_mode(x, dtype=dtype)
...
def checkfit(inputData, inputFit, epsilon, delta, BisquareLimit=6.0):
    ...
    deviation = data - fit
    sigma = robust_std(deviation, Zero=True)
    if sigma < epsilon:
