...
def interp_coevalcubes_inpdict(inpdict):
    ...
    for key,val in list(inpdict.items()):
        exec(key + '=val')
...
def convert_coevalcubes_to_sphere_surfaces(inpcubes, inpres, nside=None,
    ...
    if (freqs is None) and (redshifts is None):
        ...
        if freqs is not None:
            ...
            list_redshifts = [None for i in range(redshifts.size)]
            ...
            list_freqs = [None for i in range(freqs.size)]
    ...
    if parallel:
        try:
            list_inpcubes = [NP.take(inpcubes, ind, axis=los_axis) for ind in range(redshifts.size)]
