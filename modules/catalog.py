...
class SkyModel_new(object):
    ...
    __init__()     Initialize an instance of class SkyModel_new

    match()        Match the source positions in an instance of class 
                   SkyModel_new with another instance of the same class to a 
    ...
    def __init__(self, name, frequency, location, spec_type, spec_parms=None, 
        ...
        """
        --------------------------------------------------------------------------
        Initialize an instance of class SkyModel_new
        ...
        Read docstring of class SkyModel_new for details on these attributes.
        --------------------------------------------------------------------------
        """
    ...
    def match(self, other, matchrad=None, nnearest=0, maxmatches=-1):
        ...
        """
        -------------------------------------------------------------------------
        Match the source positions in an instance of class SkyModel_new with 
        another instance of the same class to a specified angular radius using
        spherematch() in the geometry module

        Inputs:

        other       [2-column numpy array instance of class SkyModel_new] Numpy
                    array with two columns specifying the source positions in 
                    the other sky model or the other instance of class 
                    SkyModel_new with which the current instance is to be 
                    matched with
        ...
        if not isinstance(other, (NP.ndarray, SkyModel_new)):
            raise TypeError('"other" must be a Nx2 numpy array or an instance of class SkyModel_new.')
        
        if isinstance(other, SkyModel_new):
            if (self.epoch == other.epoch) and (self.coords == other.coords):
                return GEOM.spherematch(self.location[:,0], self.location[:,1],
                                        other.location[:,0],
                                        other.location[:,1], matchrad,
    ...
    def subset(self, indices=None):
        ...
        Output:    [instance of class SkyModel_new] An instance of class 
                   SkyModel_new holding a subset of the sources in the current 
                   instance of class SkyModel_new 
        -------------------------------------------------------------------------
        """

        if (indices is None) or (len(indices) == 0):
            raise IndexError('No indices specified to select a sky model subset.')
        else:
            if self.spec_type == 'spectrum':
                return SkyModel_new(NP.take(self.name, indices), self.frequency, NP.take(self.location, indices, axis=0), self.spec_type, spectrum=NP.take(self.spectrum, indices, axis=0), src_shape=NP.take(self.src_shape, indices, axis=0), epoch=self.epoch, coords=self.coords)
            else:
                spec_parms = {}
                ...
                return SkyModel_new(NP.take(self.name, indices), self.frequency, NP.take(self.location, indices, axis=0), self.spec_type, spec_parms=spec_parms, src_shape=NP.take(self.src_shape, indices, axis=0), epoch=self.epoch, coords=self.coords)
